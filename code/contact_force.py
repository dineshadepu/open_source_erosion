...
class ComputeContactForceNormalsMohseniEB(Equation):
...
class ComputeContactForceDistanceAndClosestPointMohseniEB(Equation):
    def initialize(self, d_idx,
                   d_m,
...
class ComputeContactForceMohseniEB(Equation):
    ...
    def __init__(self, dest, sources, fric_coeff=0.5, kr=1e5, kf=1e3):
        ...
        super(ComputeContactForceMohseniEB, self).__init__(dest, sources)
    ...
    def post_loop(self,
        ...
        for i in range(d_total_no_bodies[0]):
            ...
            d_au_contact[d_idx] = (d_fn_x[t2] + d_ft_x[t2]) / d_m[d_idx]
            d_av_contact[d_idx] = (d_fn_y[t2] + d_ft_y[t2]) / d_m[d_idx]
            d_aw_contact[d_idx] = (d_fn_z[t2] + d_ft_z[t2]) / d_m[d_idx]


class TransferContactForceMohseniEB(Equation):
    def loop(self, d_idx, d_m, d_rho,
             s_idx, s_x, s_y, s_z, s_u, s_v, s_w,
             d_contact_force_normal_x,
             d_contact_force_normal_y,
        ...
        if d_contact_force_is_boundary[d_idx] == 1. and d_dem_id[d_idx] > s_dem_id[s_idx]:
