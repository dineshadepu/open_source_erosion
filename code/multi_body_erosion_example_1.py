...
class Dong2016CaseA1SquareParticleOnAl6061T6(Application):
    ...
    def create_particles(self):
        ...
        yc1 += max(target.y) - min(yc) + 10.1 * self.dx + self.dx * 1.
        yc += max(target.y) - min(yc) + 10.1 * self.dx

        body_id_2 = np.ones(len(xc), dtype=int) * 1
        xc2, yc2, _zs1 = rotate(xc, yc, np.zeros(len(xc)), axis=np.array([0., 0., 1.]),
                                angle=self.azimuth_theta)
        xc2 -= self.rigid_body_length * 2. + self.dx * 4.
        # Make the 2nd body more parallel
        yc2 += (max(yc1) - min(yc1))/2.
        # angle_2 = 45 / 180 * np.pi
        # xc2 += self.rigid_body_length / 2. * np.cos(angle_2)
        # yc2 += self.rigid_body_length / 2. * np.sin(angle_2)
